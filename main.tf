...
module "db_service" {
  ...
  image           = var.db_postgres_image  network_id      = docker_network.app_network.id
  env_vars = {
...
module "backend_service" {
  ...
  source          = "./modules/app_service"  service_name    = "${var.app_prefix}-backend-app-${count.index}"
  image           = docker_image.backend_app_image.name
  network_id      = docker_network.app_network.id  env_vars = {
    PORT        = "3000"
...
resource "docker_container" "nginx_proxy" {
  ...
  networks_advanced {    name    = docker_network.app_network.id
...
module "monitoring_dummy_service" {
  ...
  source          = "./modules/app_service"
  service_name    = local.monitoring_container_name  image           = "alpine/git"
